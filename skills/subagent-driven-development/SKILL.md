...
An outline plan is sufficient: pass the current task's outcome, affected
area, acceptance criteria, and dependencies to its implementer. The
implementer resolves code and test details while doing that task; missing
implementation code in the plan is not a defect.
