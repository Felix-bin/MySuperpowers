...
**Core principle:** The plan defines outcomes and acceptance criteria. Resolve
implementation details for the current task, verify its behavior, and leave
...
- Your harness has no subagent tool (see the per-platform references in
  `../references/`). Never fabricate a dispatch; run
  the plan here.
- Tasks are mostly independent — the same precondition as
  superpowers:subagent-driven-development.

An outline is sufficient input. Read the affected code and resolve details
for the current task as you implement it; keep later tasks as outlines.
Missing code, signatures, or test bodies are not plan defects.
...
Before Task 1, scan dependencies and any Interfaces blocks for conflicts.
Resolve unspecified internal interfaces during the owning task. For each
documented producer/consumer pair, write one ledger row — the two tasks, what one
...
For an outline, derive the current task's steps from its acceptance
criteria and the existing code. Use RED-GREEN order under
superpowers:test-driven-development, loaded at setup. A test
...
Before running a check, identify the expected result from the acceptance
criteria. Run the check, read its output, and compare. Three outcomes:
...
Commit at completed task boundaries, following any explicit plan instructions. A task that spans several commits
