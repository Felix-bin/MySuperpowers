> 本地精简版说明：本文保留上游自动引导架构的历史说明；其中的 bootstrap 安装要求不适用于此副本。公共工具映射已迁移到 `skills/references/`，本地入口仅注册技能。
...
2. **Tool mapping (per-harness).** Each harness needs the action vocabulary
   ...
   `skills/references/<harness>-tools.md` and/or inline in the
...
- Reference: `gemini-extension.json` (manifest, with `contextFileName`),
  ...
  reference), `skills/references/gemini-tools.md`.
...
`@./skills/references/<harness>-tools.md`); the harness loads
...
- **Shape A:** put it in `skills/references/<harness>-tools.md`.
