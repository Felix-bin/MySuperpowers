...
启动、普通请求和上下文压缩均不注入工作流。

### Tool Mapping

Skills speak in actions rather than naming any one runtime's tools. Use the reference below to map actions to your OpenCode flavor.
...
这是本地精简版的预期行为。用原生 `skill` 工具检查和调用需要的技能。
